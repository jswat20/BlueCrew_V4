-- Keep claim discovery and submission on the same status, lock, and level rules.

create or replace function public.current_umpire_can_claim_assignment(p_assignment_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    join public.games game
      on game.organization_id = assignment.organization_id
     and game.id = assignment.game_id
    ...
    where assignment.id = p_assignment_id
      ...
      and crew.active
      and game.level = any(crew.eligible_levels)
      and not assignment.locked
      and (
        assignment.status = 'open_for_claim'
        or (
          assignment.status = 'needs_assignment'
          and exists (
            select 1
            from public.assignment_claims claim
            where claim.organization_id = assignment.organization_id
              and claim.assignment_id = assignment.id
...
revoke all on function public.current_umpire_can_claim_assignment(uuid) from public;
grant execute on function public.current_umpire_can_claim_assignment(uuid) to authenticated;
  ...
  organization_id = public.current_organization_id() and (
    ...
    or (
      public.current_account_role() = 'umpire' and (
        ...
        or public.current_umpire_can_claim_assignment(id)
        or exists (
          select 1
          from public.assignment_claims claim
...
begin
  ...
  if v_assignment.locked or not (
    v_assignment.status = 'open_for_claim'
    or (
      v_assignment.status = 'needs_assignment'
      and exists (
        select 1 from public.assignment_claims claim
        where claim.organization_id = v_org
          and claim.assignment_id = v_assignment.id
          and claim.status = 'withdrawn'
      )
    )
  ) then
