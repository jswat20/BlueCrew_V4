...
function renderCrewCredentialBackFace(model) {
  ...
  return `<section class="crew-credential-face crew-credential-face-back" data-testid="crew-card-back">
    ...
    <section class="crew-credential-panel crew-credential-eligibility"><h3>Age Eligibility</h3><div>${model.levels.length ? model.levels.map(level => `<span class="settings-pill">${escapeCrewCardHtml(levelTerminologyService.format(level))}</span>`).join("") : "No eligibility recorded"}</div></section>
